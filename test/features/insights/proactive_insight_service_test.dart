...
void main() {
  ...
  test('Tier 1 relationships are grounded, directional and deduplicated', () async {
  ...
  test('Tier 2 commitment rules respect time boundaries and priority', () async {
  ...
  test('dismissal persists until material commitment evidence changes', () async {
    await _insertCommitment(
      database,
      id: 'due',
      text: 'Send the deck',
  ...
  test('mute and unmute suppress only the insight kind without deleting evidence', () async {
    await memoryRepository.saveCapture(
      id: 'left',
      mode: 'text',
      context: 'personal',
    ...
    expect(await database.select(database.memoryRelationships).get(), hasLength(1));
  ...
  test('empty evidence produces no fabricated insight and source detail resolves locally', () async {
    expect(await service.load(), isEmpty);
...
Future<void> _insertCommitment(
  ...
  return database.into(database.commitments).insert(
