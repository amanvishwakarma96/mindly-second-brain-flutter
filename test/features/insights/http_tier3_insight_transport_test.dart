...
void main() {
  ...
  const evidence = Tier3EvidenceBundle(sources: [source], totalCharacters: 17);
  const validPayload =
  ...
  test(
    'OpenAI Tier 3 request disables storage and uses strict schema',
    () async {
  ...
  test(
    'Anthropic Tier 3 request uses structured output configuration',
    () async {
      late http.Request request;
      final client = MockClient((incoming) async {
        request = incoming;
  ...
  test(
    'compatible Tier 3 request honors configured endpoint and model',
    () async {
      late http.Request request;
      final client = MockClient((incoming) async {
        request = incoming;
